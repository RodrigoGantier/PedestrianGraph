...
parser.add_argument('-i', '--train', required=True, default=True, type=bool,
...
def main():
    ...
    #     l0.append(result['l0'])
    #
    # features, labels = test_input_fn().make_one_shot_iterator().get_next()
    # with tf.Session() as sess:
    #     features_values = []
    #     label_values = []
    #     while True:
    #         try:
    #             ftrs, lbls = sess.run([features, labels])
    #             features_values.append(ftrs)
    #             label_values.append(lbls)
    #         except tf.errors.OutOfRangeError:
    #             break
    # features_values = np.append(features_values[0], features_values[1], axis=0)
    # label_values = np.append(label_values[0], label_values[1], axis=0)
    #
    # outputs = np.zeros([300, len(l0), 32])
    # for i in range(len(l0)):
    #     outputs[:, i, :] = l0[i].mean(1)
    #
    # outputs_ = np.zeros([300, 14 * len(l0), 32])
    # for i in range(len(l0)):
    #     outputs_[:, i*14:(i*14)+14, :] = l0[i]
    #
    # features_values = features_values.mean(-1)
    # _, ax = plt.subplots(4)
    # ax[0].imshow(label_values.reshape(1, -1), aspect='auto')
    # ax[1].imshow(features_values.mean(2).transpose(), aspect='auto')
    # ax[2].imshow(outputs.mean(2), aspect='auto')
    # ax[3].imshow(np.asarray(probabilities).reshape(1, -1), aspect='auto')
    # print("finish")
